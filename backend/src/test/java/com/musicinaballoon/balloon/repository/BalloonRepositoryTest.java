...
class BalloonRepositoryTest {
    ...
    @Test
    @DisplayName("findNotPickedByPickerIdOrderByCreatedAtDesc 는 유효한 입력을 받으면 풍선리스트를 반환한다.")
    void findNotPickedByPickerIdOrderByCreatedAtDesc_InvalidInputs_ReturnsBalloons() {
        // given
        ...
        // when
        List<Balloon> founds = balloonRepository.findNotPickedByPickerIdOrderByCreatedAtDesc(user.getId(), pageable);
    ...
    @Test
    @DisplayName("findNotPickedByPickerIdOrderByCreatedAtDesc 는 유저가 주운 풍선이 있으면 줍지 않은 풍선을 반환한다.")
    void findNotPickedByPickerIdOrderByCreatedAtDesc_HasPickedBalloonsByUser_ReturnNotPickedBalloons() {
        // given
        List<Balloon> balloons = balloons();
        User user = userRepository.save(userBuilder().build());
